  ...
  return (
    <div className="flex gap-3 w-fit not-prose h-fit shrink-0 grow-0 justify-center items-center sm:gap-2">
      <span className="hover:cursor-pointer rounded-full px-4 sm:px-3 sm:text-sm py-1.5 font-Satoshi font-normal bg-secondary flex-none">
        <Link className="flex justify-start items-center gap-2 hover:cursor-pointer" href="/">
          <AppsIcon /> Apps
        </Link>
      </span>
      {isMounted &&
        pathSegments.map((segment, index) => {
          ...
          return (
            <React.Fragment key={index}>
              ...
              <span className={`rounded-full py-1.5 font-satoshi ${isLast ? "font-semibold text-grey-700 text-lg" : "font-normal text-grey-500"} capitalize flex-none`}>
                {isLast ? (
