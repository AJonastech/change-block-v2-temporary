...
function EMPAQuestionsCollection() {
    const [questions, setQuestions] = useState<TQuestions[]>([
        ...EMPAQuestions,
        { id: EMPAQuestions.length + 1, text: "", created: new Date().toISOString() } // Default question for today
    ]);

    const addQuestion = () => {
        const newQuestion: TQuestions = {
    ...
    return (
        <div className="flex flex-col gap-8">
            {Object.entries(groupedQuestions).map(([dateLabel, { label, questions }], index) => {
